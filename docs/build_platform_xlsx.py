    ...
    '추천 · 상품': [
        ('추천 리스트 — Aurora top 10 (1번 영역 : 추천 탭)', [
            ('① 점수 · 등급 조회',
             ...
             '등급·종합점수·건강점수·VIP 확률'),
            ('② 동의 조회',
             '본인이 동의한 계열사(도메인) 목록 확보 — 미동의 계열사 상품은 추천에서 제외',
             'Redis 캐시 → Lambda 라이브 → DynamoDB 폴백',
             'consent:{global_id} / lifesync_onprem.consent',
             '캐시(24h TTL) hit → live(Lambda) → DDB row 있으면 전 도메인 폴백 → 셋 다 실패 시 추천 0건(fail-closed)'),
            ('③ 추천 캐시 확인',
             '6시간 이내 추천 결과가 있으면 그대로 재사용',
             'AWS ElastiCache (Redis)',
             'rec:{global_id}',
             '캐시 히트 시 SQL 단계 생략. cache 결과에도 동의 필터 적용 (stale 방어)'),
            ('④ 룰 매칭',
            ...
            ('⑤ 교차 추천 보강',
             ...
             '예: 은행 적금 → 카드·보험 추천 함께'),
            ('⑥ 상품 매칭 · 동의 필터',
             '카테고리별로 우선순위 상위 2개씩, 최대 10개 추출 + 동의 계열사로 필터링',
             'Aurora MySQL',
             'product_master + company + category',
             'JOIN 후 WHERE c.company_code IN (동의 도메인 목록) 으로 필터링. 카테고리·우선순위 기준 정렬'),
            ('⑦ 이력 적재 · 캐시 갱신',
...
API_SECTIONS = {
    ...
    '추천 · 상품': ('API — 추천 · 상품 호출', [
        ('/api/recommendations',
         '개인화 추천 top 10 (동의 필터 + 룰 + 캐시 + 상품 매칭)',
         'app.py · api_recommendations() — @require_jwt',
         'GET /api/recommendations',
         '7단계: DDB 메타 → 동의 조회(cache/live/fallback) → Redis 추천 캐시 → 룰 매칭 → 교차 추천 → 상품 JOIN+동의 도메인 필터 → 이력 적재. 캐시 TTL 6시간'),
        ('/product/<code>',
         '상품 상세 페이지 (SSR)',
         'app.py · product()',
         'GET /product/<product_code>',
         'Aurora product_master + 회사·카테고리·옵션. 옵션은 product_option_template 기반 한글 라벨로 변환 표시'),
        ('/product/<code>/apply',
         '상품 신청 폼 페이지 (SSR)',
